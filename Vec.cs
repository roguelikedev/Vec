...
namespace Utilities.Geometry
{
    ...
    //
    // a terse, convenient alternative to XNA Framework's Vector2, Vector3, Point, etc.
    ...
    public struct Vec
    {
        ...
        {
            return New(self.To4 - that.To4);
        }
        /// <summary> divide by zero all day long, nothing will happen. </summary>
        public static Vec operator /(Vec self, Vec that)
        {
            return ((Vec)(self.To4 / that.To4)).Take(f => float.IsNaN(f) ? 0 : float.IsInfinity(f) ? 0 : f);
        }
        ...
        public Vec Normalish
        {
            get
            {
                if (To4.Length() == 0) return 0;
                var rval = To4; rval.Normalize(); return rval;
            }
        }
        #endregion
